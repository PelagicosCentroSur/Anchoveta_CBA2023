...
CreaDataProybase<-function(dir.0,carpetaCBA,admb,yearProy,dir.Rdata,Hito){
  ...
  reps1a     <- reptoRlist(here(carpetaCBA,paste(admb,"11.rep",sep="")))  
  reps2a     <- reptoRlist(here(carpetaCBA,paste(admb,"12.rep",sep="")))
  reps3a     <- reptoRlist(here(carpetaCBA,paste(admb,"13.rep",sep=""))) 
  
  
  stds1     <- read.table(here(carpetaCBA,paste(admb,"11.std", sep='')),
                          header=T,sep="",na="NA",fill=T) 
  stds2     <- read.table(here(carpetaCBA,paste(admb,"12.std", sep='')),
                          header=T,sep="",na="NA",fill=T) 
  stds3     <- read.table(here(carpetaCBA,paste(admb,"13.std", sep='')),
                          header=T,sep="",na="NA",fill=T) 
  ...
  dataBDproy<-data.frame(S1=c(SSBp,bds1)/1000,
    ...
    melt(id.vars=c('years','indicador'))
  dataBD_rmsproy<-data.frame(S1=round(c(SSBp,bds1)/BRMSp,2),
                             S2=round(c(SSBp,bds2)/BRMSp,2),
                             S3=round(c(SSBp,bds3)/BRMSp,2),
                             base=round(c(SSBp/BRMSp,rep(NA,2)),2)) %>% 
  ...
  for(i in 1:n){
    std             <-read.table(here(carpetaCBA,paste(admb,"1",i,".std",sep="")),header=T,sep="",na="NA",fill=T)
...
CreaDataProybase_HITO2Y3<-function(dir.0,carpetaCBA,admb,yearProy,dir.Rdata,Hito){
  ...
  data1a        <- lisread(here(carpetaCBA,paste(admb,".dat",sep=""))) 
  ...
  q       <- seq(0.1,0.5,0.1)  # niveles de riesgo (cuantiles)
  nq      <- length(q)
  CBA_H2y3     <- matrix(ncol=nq,nrow=n)
  CBAp_H2y3    <- rep(0,n)
  CBApstd_H2y3 <- rep(0,n)
  ...
  for(i in 1:n){
    std              <-read.table(here(carpetaCBA,paste(admb,"1",i,".std",sep="")),header=T,sep="",na="NA",fill=T)
    CBAp_H2y3[i]    <-subset(std,name=="CBA_c0")$value[1]
    CBApstd_H2y3[i] <-subset(std,name=="CBA_c0")$std[1]
    for(j in 1:nq){
      CBA_H2y3[i,j] <-qnorm(q[j],CBAp_H2y3[i],CBApstd_H2y3[i])
    }
  }
  
  for(i in 1:n){for(j in 1:nq){
    buffer[i,j]<-round(1-CBA_H2y3[i,j]/CBA_H2y3[i,5],2)}}
  
  # CBA MENOS DESCARTE ----
  n<-3
  q       <- seq(0.1,0.5,0.1)  # niveles de riesgo (cuantiles)
  nq      <- length(q)
  CBAd_H2y3     <- matrix(ncol=nq,nrow=n)
  CBApd_H2y3   <- rep(0,n)
  CBApdstd_H2y3 <- rep(0,n)
  
  
  for(i in 1:n){
    std               <-read.table(here(carpetaCBA,paste(admb,"1",i,".std",sep="")),header=T,sep="",na="NA",fill=T)
    CBApd_H2y3[i]    <-subset(std,name=="CBA_c0d")$value[1]
    CBApdstd_H2y3[i] <-subset(std,name=="CBA_c0d")$std[1]
    for(j in 1:nq){
      CBAd_H2y3[i,j] <-qnorm(q[j],CBApd_H2y3[i],CBApdstd_H2y3[i])}}
  
  
  #===============================================================================
  # escenario 1
  #===============================================================================
  xca1 <-rnorm(1000, mean = CBAp_H2y3[1],sd=CBApstd_H2y3[1])
  xca  <-seq(min(xca1),max(xca1),0.5)
  yca  <-dnorm(xca, mean = CBAp_H2y3[1],sd=CBApstd_H2y3[1])
  icca <-qnorm(c(0.1,0.5,0.5),CBAp_H2y3[1],CBApstd_H2y3[1])
  xxca <-c(xca[xca>=icca[1]&xca<=icca[2]],
           rev(xca[xca>=icca[1]&xca<=icca[2]]))
  yyca <-c(yca[xca>=icca[1]&xca<=icca[2]],
           rep(0,length(yca[xca>=icca[1]&xca<=icca[2]])))
  ...
  # escenario 2
  #===============================================================================
  xcb1 <-rnorm(1000, mean = CBAp_H2y3[2],sd= CBApstd_H2y3[2])
  xcb  <-seq(min(xcb1),max(xcb1),0.5)
  ycb  <-dnorm(xcb, mean = CBAp_H2y3[2],sd= CBApstd_H2y3[2])
  iccb <-qnorm(c(0.1,0.5,0.5),CBAp_H2y3[2],CBApstd_H2y3[2])
  xxcb <-c(xcb[xcb>=iccb[1]&xcb<=iccb[2]],
           rev(xcb[xcb>=iccb[1]&xcb<=iccb[2]]))
  yycb <-c(ycb[xcb>=iccb[1]&xcb<=iccb[2]],
           rep(0,length(ycb[xcb>=iccb[1]&xcb<=iccb[2]])))
  ...
  # escenario 3
  #===============================================================================
  xcc1 <-rnorm(1000, mean = CBAp_H2y3[3],sd=CBApstd_H2y3[3])
  xcc  <-seq(min(xcc1),max(xcc1),0.5)
  ycc  <-dnorm(xcc, mean = CBAp_H2y3[3],sd=CBApstd_H2y3[3])
  iccc <-qnorm(c(0.1,0.5,0.5),CBAp_H2y3[3],CBApstd_H2y3[3])
  xxcc <-c(xcc[xcc>=iccc[1]&xcc<=iccc[2]],
           rev(xcc[xcc>=iccc[1]&xcc<=iccc[2]]))
  yycc <-c(ycc[xcc>=iccc[1]&xcc<=iccc[2]],
           rep(0,length(ycc[xcc>=iccc[1]&xcc<=iccc[2]])))
  ...
  q       <- seq(0.1,0.5,0.1)  # niveles de riesgo (cuantiles)
  nq      <- length(q)
  CBA_H2y3D2SR     <- matrix(ncol=nq,nrow=n)
  CBAp_H2y3D2SR    <- rep(0,n)
  CBApstd_H2y3D2SR <- rep(0,n)
  ...
  for(i in 1:n){
    std              <-read.table(here(carpetaCBA,paste(admb,"1",i,".std",sep="")),header=T,sep="",na="NA",fill=T)
    CBAp_H2y3D2SR[i]    <-subset(std,name=="CBA_c0R")$value[1]
    CBApstd_H2y3D2SR[i] <-subset(std,name=="CBA_c0R")$std[1]
    for(j in 1:nq){
      CBA_H2y3D2SR[i,j] <-qnorm(q[j],CBAp_H2y3D2SR[i],CBApstd_H2y3D2SR[i])
    }
  }
  
  for(i in 1:n){for(j in 1:nq){
    bufferD2SR[i,j]<-round(1-CBA_H2y3D2SR[i,j]/CBA_H2y3D2SR[i,5],2)}}
  
  # CBA MENOS DESCARTE ----
  n<-3
  q       <- seq(0.1,0.5,0.1)  # niveles de riesgo (cuantiles)
  nq      <- length(q)
  CBAd_H2y3D2SdR     <- matrix(ncol=nq,nrow=n)
  CBApd_H2y3D2SdR    <- rep(0,n)
  CBApdstd_H2y3D2SdR <- rep(0,n)
  
  
  for(i in 1:n){
    std               <-read.table(here(carpetaCBA,paste(admb,"1",i,".std",sep="")),header=T,sep="",na="NA",fill=T)
    CBApd_H2y3D2SdR[i]    <-subset(std,name=="CBA_c0dR")$value[1]
    CBApdstd_H2y3D2SdR[i] <-subset(std,name=="CBA_c0dR")$std[1]
    for(j in 1:nq){
      CBAd_H2y3D2SdR[i,j] <-qnorm(q[j],CBApd_H2y3D2SdR[i],CBApdstd_H2y3D2SdR[i])}}
